"""Smoke tests for the agenda CLI entry point and its verbs."""
...
from agenda.explain import known_paths
...
def test_no_args_prints_help(capsys: pytest.CaptureFixture[str]) -> None:
    rc = main([])
    assert rc == 0
    assert "usage: agenda" in capsys.readouterr().out
...
def test_explain_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert "agenda whoami" in payload["markdown"]
