"""``agenda learn`` — the learnability affordance.
...
from agenda.cli._output import emit_result

_TEXT = """\
agenda — a clonable template for AgentCulture mesh agents.
...
Commands
--------
  agenda whoami             Identity from culture.yaml.
  agenda learn              This self-teaching prompt.
  agenda explain <path>...  Markdown docs for any noun/verb path.
  agenda overview           Descriptive snapshot of the agent.
  agenda doctor             Check the agent-identity invariants.
  agenda cli overview       Describe the CLI surface itself.
...
def _as_json_payload() -> dict[str, object]:
    return {
        ...
        "json_support": True,
        "explain_pointer": "agenda explain <path>",
    }
