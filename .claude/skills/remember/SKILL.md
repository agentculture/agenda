...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope agenda
...
scope:** the default personal scope is **private** (`--scope agenda
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `agenda`). Private records are served only to a recall in the same
