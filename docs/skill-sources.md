...
agenda vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. agenda
...
#   - consumer-identifying prose: `guildmaster` → `agenda` (NOT
...
If a re-sync would lose a agenda adaptation, lift the change
